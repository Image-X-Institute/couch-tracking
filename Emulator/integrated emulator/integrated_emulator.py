...
save_folder = r"C:\Users\imagex_labl\Documents\Elisa\Experiments\No_7_emulator\integrated emulator"
...
class MotionCompensationSimulator:
    ...
    def run_simulation(self):
        ...
        self.feedback_log_path = os.path.join(save_folder,self.log_file_path.replace("simulation_log", "feedback_log"))
        ...
        self.log_file_path = os.path.join(save_folder, self.log_file_path)
    ...
    def plot_results(self, plot_motor_position=True):
        ...
        # Basic metrics
        self.residual_error = [ty - cy for ty, cy in zip(true_y, corrected_y)]
        self.compensated = [sy for sy in sent_y]
        ...
        # --- title pieces ---
        trace_label = os.path.splitext(getattr(self, "trace_name", os.path.basename(self.log_file_path)))[0]
        ...
        plt.plot(time_vals, true_y, label="Original Motion", linewidth=1, markersize=3)
        plt.plot(time_vals, sent_y, label="Compensated motion", linestyle="--", linewidth=1, markersize=3)


        if (motor_t is not None) and (motor_pos is not None):
            # scatter to avoid line artifacts
            plt.scatter(motor_t, motor_pos, s=2, color = 'green', label="Couch position")
        ...
        out_name = os.path.join(save_folder, f"plot_{trace_label}_{int(self.latency*1000)}ms_{self.motor_velocity}mm_per_s_{track_dt_ms}ms_{time.strftime('%Y%m%d_%H%M%S')}.png".replace(" ", "_"))
        ...
        print("Saved plot to:", os.path.abspath(out_name))
        print(f"Max Error: {max(abs(e) for e in self.residual_error):.2f} mm")
        print(f"RMS Error: {np.sqrt(np.mean(np.square(self.residual_error))):.2f} mm")
...
def run_emulator(args):
    ...
    log_file = f"{trace_name}_simulation_log_{args.mode}_{latency_ms}ms_{args.velocity}mm_per_s_{str(int(args.track_dt * 1000)).replace('.', '_')}ms.txt"
    
    sim = MotionCompensationSimulator(
        trace_file=args.trace,
        latency=args.latency,
        local_ip="127.0.0.1" if args.mode == "local" else "0.0.0.0",
        local_port=args.local_port,
        target_ip="127.0.0.1" if args.mode == "local" else args.pi_ip,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--local-port", type=int, default=2400)
    parser.add_argument("--target-port", type=int, default=1400)
    parser.add_argument("--velocity", type=float, default=12.0)
