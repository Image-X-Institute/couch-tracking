...
const double MOTOR_VELOCITY = 100.0; // Simulate motor velocity here, mm/s
...
void openLogFile() {
    logFile.open("EMULATOR_SA--.txt", ios::out);
    if (!logFile.is_open()) {
...
// Add global parameters for tunable control
const double GAIN = 0.9; // Proportional gain, 0 < GAIN < 1
const double FILTER = 0.5; // In mm; do not correct small errors
const double ACTUATION_LAG_MS = 0; // In ms, Delay before motor starts moving
const double SMOOTHING_FACTOR = 0.6; // Weight for low-pass filter effect
...
double Move(double depth, double error) //parameter in mm
{
    ...
    // Apply motor lag
    usleep(static_cast<useconds_t>(ACTUATION_LAG_MS * 1000));
    ...
    usleep(static_cast<useconds_t>(move_duration_ms*1000)); // Simulate the motor movement
...
void UDPSendMotorMotion(double error, const sockaddr_in& client_addr) {
    ...
    send_addr.sin_port = client_addr.sin_port;  // 
    ...
    if (sent < 0) {
        ...
        std::cout << "[Couch] Sent motor compensation feedback: " << compensation << std::endl;
        logFile << duration_cast<milliseconds>(recv_time.time_since_epoch()).count()
                << ", Sent feedback: " << compensation << endl;
    }
    close(sock);
}
...
void processMotorCommandPosition(PositionData position, double offset, const sockaddr_in& client_addr) {
    ...
    // Deadband
    if (std::abs(error) < FILTER || std::abs(error)>40.0)
	return;
    
    // Apply gain
    double error_to_compensate = GAIN * error;
    adjusted += SMOOTHING_FACTOR * (-error_to_compensate);
    auto recv_time = high_resolution_clock::now();
    double duration = (Move(adjusted, error_to_compensate));
    auto end_time = high_resolution_clock::now();

    logFile << duration_cast<microseconds>(end_time.time_since_epoch()).count() << " " << target << " " << adjusted << " Move" << endl;
	                    
    // Target tracking delay
    ...
    UDPSendMotorMotion( (SMOOTHING_FACTOR*error_to_compensate), client_addr); // Send the motor displacement via UDP 
