...
// Motor command function to replicate incoming position.x directly
void processMotorCommand(PositionData position) {
    double target = position.x; // Get the target position from incoming data
    
    std::cout << "Target position: " << target << " mm" << std::endl;
	double adjustedTarget = home_pos + target;
    // Move the motor to the incoming position directly
    auto recv_time = high_resolution_clock::now();
    auto end_Time = Move(adjustedTarget, recv_time);  // Move the motor directly to target position

    logFile << duration_cast<microseconds>(recv_time.time_since_epoch()).count() << " " << adjustedTarget << " " << target << " " << "Move" << std::endl;
}
