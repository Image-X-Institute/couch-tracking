...
std::queue<timewDepth> dataQueue; // First in, first out data strucutre. Element is added to the back of the queue
...
void receiveUDPData(int socket, struct sockaddr_in* si_other, socklen_t* slen) {
    char buf[SIZE];
    while (true) {
        memset(buf, '\0', SIZE);
	    ...
	    std::cout << "Frame No: " << info.time << ", Depth: " << info.depth/100 << "mm" << std::endl;
...
void processMotorCommand(timewDepth info, double offset) {
    ...
    if (abs(depth_mm - offset) <= 40) {
	    ...
	    std::cout << "XXX Start Moving XXX " << info.time << std::endl;
	    std::cout << "error is: " << error << std::endl;

            adjusted = adjusted - error;
            
            std::cout << "Depth is: " << depth_mm << ", adjusted couch position: " << adjusted << std::endl;
            // Get the current timestamp
	    auto recv_time = high_resolution_clock::now();
	    auto end_Time = Move(adjusted, recv_time);
	    std::cout << "XXX End Moving XXX " << info.time <<std::endl;
        ...
        logFile << info.time << " " << duration_cast<microseconds>(out_time.time_since_epoch()).count() << " " << depth_mm << " " << "N/A" << " " << "Noise" <<std::endl;
    }
    } else {
	auto start_time = high_resolution_clock::now();
        std::cout << "Reading exceeds limit." << (abs(depth_mm - offset)) <<std::endl;
        logFile << info.time << " " << duration_cast<microseconds>(start_time.time_since_epoch()).count() << " " << depth_mm << " " << "N/A" << " " << "Large" <<std::endl;
    }
}
...
void motorControl() {
    while (true) {
	    ...
	    timewDepth currentData = latestDepth; // Get the latest data point
	    lock.unlock();
	    ...
	    logFile << latestDepth.time << " " << duration_cast<microseconds>(in_time.time_since_epoch()).count() << " " <<latestDepth.depth << " " << "N/A"  << " " << "SKIP" <<std::endl;
            lock.unlock();
        }
    }
}
...
int main() {
  ...
  openLogFile();
    ...
    std::thread udpThread(receiveUDPData, s, &si_other, &slen);
