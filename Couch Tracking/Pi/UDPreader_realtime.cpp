	...
	while (true) {
		...
		*/
		...
		if (buf[0] == 't') {
			...
			// Check if the depth measurement is within the acceptable range from the offset value
			if (abs(depth - offset) <= 50)
			{
			    // Check if the current reading differs from the previous reading by 0.3mm
			    if (abs(depth - prevDepth) >= 1)
			    {
				double adjusted = home_pos - (depth - offset);
				Move(adjusted);
				prevDepth = depth;
			    }
			    else
			    {
				cout << "Ignore small change." << endl;
			    }
			}
			else
			{
			    cout << "Reading exceeds limit." << endl;
			}
