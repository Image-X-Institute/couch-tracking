...
int main(void)
{
    ...
    newMessage("Waiting for console input...");
    LoggerMain("CouchTrackingLog");
    vector<thread> threads;
    stringstream msgSS;
    
    // Main loop to accept console input
    while (true)
    {
	double depth;
	cout << "Enter depth (in mm): ";
	cin >> depth;
	...
	{
	    stringstream textSS;
	    newMessage(textSS.str());
	    Move(depth);
	}));
    }
    return 0;
    //check if the step length should be calibrated
    /*char decision;
    cout << "Do you want to calibrate the steps/mm? If so enter 'y', otherwise press any key to continue" << endl;
    cin >> decision;
    if(decision == 'y')
    {
	stepsPerMill = repeatTest(2);
    }*/
    ...
    cin.clear();
    string serverIn;
    cout << "Press any key to begin listening...";
    getline(cin,serverIn);
    ...
    //si_me.sin_addr.s_addr = inet_addr("10.17.192.172");
    const char* receiverIpAddress = "10.17.192.172";
    
    inet_pton(AF_INET, receiverIpAddress, &(si_me.sin_addr));
    
    if (bind(s, (struct sockaddr *)&si_me, sizeof(si_me))==-1)
	    printf("bind failed. (incoming)\n\n");
	    
    cout << "UDP receiver is listening on IP address: " << inet_ntoa(si_me.sin_addr) << endl;
    cout << "UDP receiver is listening on port: " << ntohs(si_me.sin_port) << endl;
    
    newMessage("Socket created, UDP listening...");
    LoggerMain("CouchTrackerLog");	
    vector<thread> threads;
    stringstream msgSS;
    do
    {
	...
	if (buf[0] == 't')
	{ 
	    double depth = BufferToPosition(buf);//convert the buffer to depth [mm]
