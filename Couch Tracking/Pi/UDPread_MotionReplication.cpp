...
const char* ipAddress = "192.168.8.141"; // This address is pre-set with router
...
//////////////////////////////
// Depth Data Processing - Motor command to replicate incoming position directly
...
void processMotorCommandDepth(timewDepth info, double offset) {
    ...
    if (abs(depth_mm - offset) <= 40) {
        if (abs(depth_mm - offset) >= 0) {
            double error = offset - depth_mm;
	    adjusted = home_pos - error;
	    	    
	    auto recv_time = system_clock::now();
            double duration = Move(adjusted, recv_time);
	    auto timestamp = system_clock::now();
	            
            logFile << info.time << " " << duration_cast<microseconds>(timestamp.time_since_epoch()).count() << " " << depth_mm << " " << adjusted << " Move" << endl;
        }
    }
}
...
void receiveUDPDepthData(int socket, struct sockaddr_in* si_other, socklen_t* slen) {
	...
	logFile << info.time<<" " << duration_cast<microseconds>(receive_time.time_since_epoch()).count() << " " << info.depth/100 << adjusted << " Received data" <<std::endl;
...
int main() {
    ...
    si_me.sin_addr.s_addr = inet_addr(ipAddress); // Give IP address here "192.168.8.141"
