...
.csv file generated by Intel RealSense depth camera. There are 5 columns in the file: frame number, timestamp from realsense library, timestamp from local system, ROI 1 depth measurement (measuring tracking target), ROI 2 depth measurement (measuring couch motion). The first 30 depth camera frames are collected to calculate the averaged absolute position of tracking target, and being used as the isocenter (offset) value. If tracking target deviated from isocenter, couch compensation kicks in.
