...
FILES = {
    "sin_5-1" : ROOT /"sin_5_comp-1.csv",    "sin_5-2" : ROOT /"sin_5_comp-2.csv",    "sin_5-3" : ROOT /"sin_5_comp-3.csv",
    "sin_8-1" : ROOT /"sin_8_comp-1.csv",    "sin_8-2" : ROOT /"sin_8_comp-2.csv",    "sin_8-3" : ROOT /"sin_8_comp-3.csv",
    "sin_13-1" : ROOT /"sin_13_comp-1.csv",  "sin_13-2" : ROOT /"sin_13_comp-2.csv",  "sin_13-3" : ROOT /"sin_13_comp-3.csv",
    }
KEY = "sin_5-3" # <-- change this one line
FILEPATH = FILES[KEY]
...
plt.plot(time, distance1, linestyle='--', color='blue', label='Target')
plt.plot(time, distance2, linestyle='--', color='black', label='Couch')
plt.plot(t_fit, distance1_fit, 'r-', label='Fitted Target Sine Wave')
plt.plot(peak_times1, peak_values1, 'ro', label='Target\'s Peaks')
plt.plot(t_fit, distance2_fit, 'g-', label='Fitted Couch Sine Wave')
plt.plot(peak_times2, peak_values2, 'go', label='Couch\'s Peaks')

# Add vertical line and print time for 3rd peak of Distance 1
if len(peak_times1) >= 3:
    pt = peak_times1[2]
    pv = peak_values1[2]
    plt.axvline(x=pt, color='red', linestyle='-', alpha=0.6)
    plt.text(pt + 1.5, pv, f"{pt:.3f}s", rotation=0, color='red', fontsize=10)

# Add vertical line and print time for 3rd peak of Distance 2
if len(peak_times2) >= 3:
    pt = peak_times2[2]
    pv = peak_values2[2]
    plt.axvline(x=pt, color='green', linestyle='-', alpha=0.6)
    plt.text(pt + 1, pv, f"{pt:.3f}s", rotation=0, color='green', fontsize=10)

plt.title(BASE_NAME, fontsize=18)
plt.legend()
